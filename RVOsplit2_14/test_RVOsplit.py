...
env = Simple_test_env("RVOsplit2_14")
LOW = 0; HIGH = 1000
INRANGES=[(0,5),(45,50)]
TESTS = 100000
KEEP_SPLITTING=True
...
def RVOsplit():
    ...
    while(True):
        ...
        for parti,part in random.sample(list(enumerate(parts)),len(parts)):
            if (not isvalid[parti-1] or not isvalid[(parti+1)%len(parts)]) and isvalid[parti]:
                i,a=sample(part)
                if validsam(a):
                    print((a,iters)); return a
        for parti, part in random.sample(list(enumerate(parts)), len(parts)):
            if isvalid[parti - 1] and isvalid[parti] and isvalid[(parti + 1) % len(parts)]:
                i, a = sample(part)
                if validsam(a):
                    print((a, iters)); return a
        for part in parts:
