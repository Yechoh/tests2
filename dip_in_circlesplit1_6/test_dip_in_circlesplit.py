...
env = Simple_test_env("dip_in_circlesplit1_6")
TEST=True
...
def circlesplit(ourparts,ourodds,otherparts=[],otherodds=[]):
    ...
    for i in range(len(ourparts)):
        if ourparts[i]:
            totalodds[i] = Fraction(1, len(ourparts)) + Fraction(len(ourparts) - nvars, len(ourparts)) * totalodds[i]
